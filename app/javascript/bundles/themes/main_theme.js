class Theme {
  ...
  indeterminateCheckboxes() {
    $('input[type="checkbox"][indeterminate], input[type="checkbox"][data-indeterminate="true"]').prop(
      'indeterminate',
      true
    );
  }
    ...
    if (window.Chart) {
      ...
      const settings = {
        global: {
          ...
          defaultColor: isDarkSkin ? this.hexToRgba(colors.white, 0.08) : this.hexToRgba(colors.black, 0.1),
          ...
          tooltips: {
            backgroundColor: isDarkSkin ? this.hexToRgba(colors.white, 0.98) : this.hexToRgba(colors.black, 0.98),
            xPadding: 8,
            yPadding: 8,
            titleMarginBottom: 8,
            footerMarginTop: 8,
        ...
        scale: {
          gridLines: {
            color: isDarkSkin ? this.hexToRgba(colors.white, 0.08) : this.hexToRgba(colors.black, 0.1),
            zeroLineColor: isDarkSkin ? this.hexToRgba(colors.white, 0.08) : this.hexToRgba(colors.black, 0.1)
          }
        }
      };
  ...
  flotDefaultOptions() {
    ...
    return {
      xaxis: {
        tickColor: isDarkSkin ? this.hexToRgba(colors.white, 0.08) : this.hexToRgba(colors.black, 0.1),
  ...
  easypie() {
    if ($.fn.easyPieChart) {
      ...
      $('[data-toggle="easypiechart"]').each(function() {
        ...
        options.trackColor =
          options.trackColor || self.skin === 'dark' ? self.getColors('gray')[200] : self.colors.white;
      ...
      $('[data-toggle="tribute"]').each(function() {
        ...
        if (options.itemTemplate == true) {
          options.menuItemTemplate = item => {
            return `<span class="user-avatar user-avatar-sm mr-2"><img src="${item.original.avatar}"></span> ${
              item.string
            }`;
          };
        }
      ...
      $('[data-mask]').each(function() {
        ...
        if (options.autoCorrectDate) {
          const autoCorrectedDatePipe = textMaskAddons.createAutoCorrectedDatePipe
            ? textMaskAddons.createAutoCorrectedDatePipe(options.autoCorrectDate)
            : null;
