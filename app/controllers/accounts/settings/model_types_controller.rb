  ...
  module Settings
    class ModelTypesController < Accounts::BaseController
      ...
      def update
        respond_to do |format|
          if @model_type.update(model_type_params)
            format.html { redirect_to edit_settings_model_type_path(@model_type), notice: I18n.t('cfields.flash_updated') }
      ...
      def destroy
        @model_type.destroy
        respond_to do |format|
          format.html { redirect_to settings_model_types_path, notice: 'Model type was successfully destroyed.' }
