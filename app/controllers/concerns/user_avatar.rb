module UserAvatar
  ...
  def alphatar_for(user = nil, classnames = nil, id = nil, js = false)
    if js
      ...
      content_tag :div, "#{user.first_name.upcase.first}#{user.last_name.upcase.first}", class: "alphatar #{classnames}", style: "background-color: #{user.try(:color) || '#404E5C'}", alt: "profile-pic", id: "#{id}"
    end
  end
