        ...
        if not test(%Q[[ -e "#{shared_path.join(".schema_loaded")}" ]])
          within release_path do
            with rails_env: fetch(:rails_env) do
              sh 'bin/rails runner ./lib/scripts/areas_preloader.rb'
              sh 'bin/rails runner ./lib/scripts/categories_preloader.rb'
              sh 'bin/rails runner ./lib/scripts/extras_preloader.rb' # Load any extras
